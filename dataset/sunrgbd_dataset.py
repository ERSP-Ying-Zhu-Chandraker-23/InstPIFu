        ...
        if testid is not None:
            ...
            for item in self.split:
                ...
                if id==testid:
                    ...
                    num_boxes = len(sequence['boxes']['bdb2D_pos'])
                    for i in range(num_boxes):
                        new_split.append([item, i])
                    ### \NEW CODE
    ...
    def __getitem__(self, index):
        ...
        if isinstance(self.split[index],list):
            ...
            if self.config['data']['use_pred_pose']:
                print("Inner if statement entered")
                ...
                print("PRED_PITCH for objidx %d: %f" % (object_id, pred_pitch))
            ...
            print("Inner if statement completed")
            ...
            # orientation=[pitch,roll,yaw] # this is only used in box_info, which is never accessed
            ...
            '''
            box_info={ # this is never accessed
            ...
            data_batch= {"whole_image":objrecon_image.float(),'bg_image':bg_image.float(),'image':patch.float(),'bg_intrinsic':bg_intrinsic,'depth':depth, "K":camera['K'],"intrinsic":camera["K"],'bbox_size':bbox_size,'bdb2D_pos':bdb,
                        ...
                        'cls_codes':cls_codes,'rel_coord':rel_coord,'bdb_grid':bdb_grid} #old data batch with obj_cam_center and rot_matrix
